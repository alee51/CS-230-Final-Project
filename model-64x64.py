...
class Config:
    data_root = "."
    image_size = 64                # [may test on 64x64 instead of 256x256 first]
    train_split = 0.9               # 90% train, 10% val
    batch_size = 8                  # adjust for GPU; 8–16 recommended
    lr = 1e-4
    weight_decay = 0.01
...
def build_split_lists(root_dir, seed=42):
    train_root = Path(f'{root_dir}/1/train_64x64/train_64x64')
    val_root = Path(f'{root_dir}/1/valid_64x64/valid_64x64')
    
    print(train_root, len([f for f in train_root.iterdir()]))
    train_files = [str(train_root/f.name) for f in train_root.iterdir() if f.is_file()]
    print(val_root, len([f for f in val_root.iterdir()]))
    val_files = [str(val_root/f.name) for f in val_root.iterdir() if f.is_file()]
    ...
    train_items = [(f,0) for f in train_files]
    val_items = [(f,0) for f in val_files]
...
train_items, val_items = build_split_lists(cfg.data_root)
...
model = UNet2DModel(
    ...
    in_channels=3,
    out_channels=3,
    block_out_channels=(64, 32, 32),  # deeper for 256x256
...
def predict_x0_from_eps(x_t, t, eps, scheduler):
    alpha_prod_t = scheduler.alphas_cumprod[t.to('cpu')].to(x_t.device)
